...
import { calculateEscalatorBoost, animateActiveEscalatorSteps } from './escalatorTMPbroken.js';
...
function generateWorld() {
    ...
    const NegativeZ = -16; // Offset for B-Wing structures, matches usage elsewhere
 ...
 */
        ...
        if (i > -SETTINGS.numBasementFloors && i <= SETTINGS.numFloors -1 ) { // Allow escalators from ground to basement, and between above-ground floors
            ...
            startEscDown.name = `Left Escalator Wing-A  Down Start ${i}`;
            ...
            for (let s = 0; s < stepCount; s++) {
                ...
                stepDown.name = `Left Escalator Wing-A  Step Down ${i}-${s}`;
                scene.add(stepDown);
            ...
            endEscDown.name = `Left Escalator Wing-A  Down End ${i}`;
            //start1Esc.rotation.x = -Math.PI / 2;
            ...
            // --- End of Wing-A Left side Escalator Down on lower floor Ending Point --- ///
            ...
            startEscUp.name = `Right Escalator Wing-A  Up Start ${i}`;
            //start1Esc.rotation.x = -Math.PI / 2;
            ...
            for (let s = 0; s < stepCount; s++) {
                ...
                stepUp.name = `Right Escalator Wing-A  Step Up ${i}-${s}`;
                scene.add(stepUp);
                worldObjects.push(stepUp);
                escalatorSteps.up[i].push(stepUp); // Track stepUp
            }

            // Wing-A  Escalator Up from lower floor Ending Point    
            ...
            endEscUp.name = `Right Escalator Wing-A  Up End ${i}`;
            //start1Esc.rotation.x = -Math.PI / 2;
            ...
            // End of Wing-A Right side escalator Ramp going up from lower floor////
            ...
            const startEscDownBR = new THREE.Mesh(startEscDownBRGeo, EscalatorEmbarkMaterial);
            startEscDownBR.name = `Right B Escalator Down Start ${i}`;
            startEscDownBR.position.set(
                - (escalatorWidth / 2) - 0.1, // was settings.corridorWidth + (escalatorWidth / 2) + 0.1,
                floorY - (floorDepth / 2),
                NegativeZ - totalCorridorLength - 3.5 // Adjusted: further in -Z for start of "down"
            );
            startEscDownBR.receiveShadow = true;
            scene.add(startEscDownBR);
            worldObjects.push(startEscDownBR);
            escalatorStartsB.down[i] = startEscDownBR;
            ...
            for (let s = 0; s < stepCount; s++) {
                ...
                const stepDownB = new THREE.Mesh(stepBGeo, EscalatorMaterial);
                ...
                stepDownB.name = `Right B Escalator Step Down ${i}-${s}`;
                scene.add(stepDownB);
                worldObjects.push(stepDownB);
                escalatorStepsB.down[i].push(stepDownB);
            }
            ...
            endEscDownB.name = `Right B Escalator Down End ${i}`;
            endEscDownB.position.set(
                - (escalatorWidth / 2) - 0.1,// was settings.corridorWidth + (escalatorWidth / 2) + 0.1,
                floorY - SETTINGS.floorHeight - (floorDepth / 2),
                NegativeZ - totalCorridorLength - escalatorLength - 4 - 0.5
            );
            ...
            startEscUpB.name = `Left B Escalator Up Start ${i}`;
            ...
            for (let s = 0; s < stepCount; s++) {
                ...
                stepUpB.name = `Left B Escalator Step Up ${i}-${s}`;
                scene.add(stepUpB);
                worldObjects.push(stepUpB);
                escalatorStepsB.up[i].push(stepUpB);
            }
            ...
            endEscUpB.name = `Left B Escalator Up End ${i}`;
            endEscUpB.position.set(
                SETTINGS.corridorWidth + 0.1 + (escalatorWidth / 2),
                floorY - (floorDepth / 3) - 0.08,
                NegativeZ - totalCorridorLength - 3.5 // Adjusted: further in -Z for end of "up"
            );
            ...
            translatedEndEscUpB.position.z -= 0.3; // Keep this relative adjustment
            ...
            // Wing-A  Balustrades for Escalator UP (Left side, X from -escalatorWidth to 0)
            ...
            innerBalustradeUp.name = `Wing-A Balustrade_Up_Inner_F${i-1}-F${i}`;
            ...
            outerBalustradeUp.name = `Wing-A Balustrade_Up_Outer_F${i-1}-F${i}`;
            ...
            // Wing-A  Balustrades for Escalator DOWN (Right side, X from SETTINGS.corridorWidth to SETTINGS.corridorWidth + escalatorWidth)
            ...
            innerBalustradeDown.name = `Wing-A Balustrade_Down_Inner_F${i}-F${i-1}`;
            ...
            outerBalustradeDown.name = `Wing-A Balustrade_Down_Outer_F${i}-F${i-1}`;
            ...
            const startUpBalustradeB = new THREE.Vector3(
                (SETTINGS.corridorWidth/2)+SETTINGS.escalatorWidth / 2, // This X seems off, B-Up is on CorridorWidth + side
            ...
            const endUpBalustradeB = new THREE.Vector3(
                (SETTINGS.corridorWidth/2)+SETTINGS.escalatorWidth / 2, // This X seems off
                currentFloorTopY - floorDepth / 2, 
                NegativeZ - (totalCorridorLength + 3.5)
            );
            const dirUpBalustradeB = new THREE.Vector3().subVectors(endUpBalustradeB, startUpBalustradeB); // Use B vectors
            const lengthUpBalustradeB = dirUpBalustradeB.length(); // Use B dir
            const centerPosUpBalustradeB = new THREE.Vector3().addVectors(startUpBalustradeB, endUpBalustradeB).multiplyScalar(0.5); // Use B vectors
            const centerZ_UpBalustradeB = centerPosUpBalustradeB.z; // Use B center
            const rampSurfaceY_at_centerZ_UpB = startUpBalustradeB.y + (centerZ_UpBalustradeB - startUpBalustradeB.z) / (endUpBalustradeB.z - startUpBalustradeB.z) * (endUpBalustradeB.y - startUpBalustradeB.y); // Use B vectors
            ...
            innerBalustradeUpB.position.set(
                ...
                centerPosUpBalustradeB.z // Use B-wing Z center
            );
            ...
            const upDirB = dirUpBalustradeB.clone().normalize(); // Use already computed B dir
            ...
            [innerBalustradeUpB, outerBalustradeUpB].forEach(balustrade => {
                ...
                const cylinder1 = new THREE.Mesh(cylinderGeoB, balustradeMaterial); // Use cylinderGeoB
                cylinder1.position.copy(end1);
                const cylinder2 = new THREE.Mesh(cylinderGeoB, balustradeMaterial); // Use cylinderGeoB
            ...
            const centerPosDownBalustradeB = new THREE.Vector3().addVectors(startDownBalustradeB, endDownBalustradeB).multiplyScalar(0.5); // Use B vectors
            const centerZ_DownBalustradeB = centerPosDownBalustradeB.z;
            const rampSurfaceY_at_centerZ_DownB = startDownBalustradeB.y + (centerZ_DownBalustradeB - startDownBalustradeB.z) / (endDownBalustradeB.z - startDownBalustradeB.z) * (endDownBalustradeB.y - startDownBalustradeB.y); // Use B vectors
            ...
            innerBalustradeDownB.position.set(
                ...
                centerPosDownBalustradeB.z // Use B-wing Z center
            );
            ...
            const downDirB = dirDownBalustradeB.clone().normalize(); // Use already computed B dir
