 ...
 */
           ...
           /*  if (frontWallRightWidth > 0.01) {
            ...
            for (let j = 0; j < SETTINGS.doorsPerSide; j++) {
                ...
                const chairBZ_R = 0.1 + (deskRB.position.z + backWallZ_B_R_Chair) / 2; // Position chair between desk and back wall
                const chairBX_R = -(SETTINGS.roomSize/2);
                const chairBY_R = floorY + chairSeatHeight / 2;
                const chairBSeat_R = new THREE.Mesh(new THREE.BoxGeometry(chairSeatWidth, chairSeatHeight, chairSeatDepth), deskMaterial);
                chairBSeat_R.position.set(chairBX_R, chairBY_R, chairBZ_R); // scene.add(chairBSeat_R); worldObjects.push(chairBSeat_R);
                ...
                backrestB_R.position.set(chairBX_R, chairBY_R + chairBackrestHeight / 2, chairBZ_R + chairSeatDepth / 2 - chairBackrestThickness / 2);
                // scene.add(backrestB_R); worldObjects.push(backrestB_R);
                
                const rightRoomBContents = new THREE.Group();
                const rightRoomBId = `B_R_F${i}_D${j}`;
                rightRoomBContents.name = `RoomContents_B_${rightRoomBId}`;
                rightRoomBContents.add(rFloorB); worldObjects.push(rFloorB); // Add to worldObjects for collision if needed
                rightRoomBContents.add(rCeilingB); worldObjects.push(rCeilingB);
                rightRoomBContents.add(deskRB); worldObjects.push(deskRB);
                rightRoomBContents.add(cabinetRB); worldObjects.push(cabinetRB);
                rightRoomBContents.add(chairBSeat_R); worldObjects.push(chairBSeat_R);
                rightRoomBContents.add(backrestB_R); worldObjects.push(backrestB_R);
                ...
                if (isRightBRoomRedDoor) {
                    ...
                    rightRoomBContents.add(safeBR); worldObjects.push(safeBR);
                ...
                rightRoomBContents.visible = false;
                scene.add(rightRoomBContents);

                allRoomsData.push({
                    id: rightRoomBId,
                    door: null, windowGlass: null, opaqueMaterial: null, transparentMaterial: null, contentsGroup: rightRoomBContents,
                ...
                const leftRoomBContents = new THREE.Group();
                const leftRoomBId = `B_L_F${i}_D${j}`;
                leftRoomBContents.name = `RoomContents_${leftRoomBId}`;
                leftRoomBContents.add(lFloorB); worldObjects.push(lFloorB);
                leftRoomBContents.add(lCeilingB); worldObjects.push(lCeilingB);
                leftRoomBContents.add(deskBL); worldObjects.push(deskBL);
                leftRoomBContents.add(cabinetBL); worldObjects.push(cabinetBL);
                leftRoomBContents.add(chairSeat_BL); worldObjects.push(chairSeat_BL);
                leftRoomBContents.add(backrest_BL); worldObjects.push(backrest_BL);
                ...
                if (isLeftRoomRedDoor) {
                    ...
                    leftRoomBContents.add(safeBL); worldObjects.push(safeBL);
                ...
                leftRoomBContents.add(roomLampBL);
                ...
                leftRoomBContents.visible = false;
                scene.add(leftRoomBContents);
                allRoomsData.push({ // Ensure new properties are initialized
                    id: leftRoomBId, door: null, windowGlass: null, opaqueMaterial: null, transparentMaterial: null, contentsGroup: leftRoomContents,
                    visibleByDoor: false, visibleByWindow: false, lamp: roomLampL
                });
            }
            ...
            const escLightBPositions = [escBBridgeStartRefZ, escBBridgeStartRefZ - (2*escalatorLength/3)];
            escLightBPositions.forEach((zPos, idx) => {
                // escalator bridge light creation logic
            ...
            const wallBL3 = new THREE.Mesh(wallL3Geo, wallMaterial);
