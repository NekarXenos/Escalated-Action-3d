...
import { createEscalatorsForFloor, updateEscalatorStepVisuals, calculateEscalatorBoost, animateActiveEscalatorSteps } from './escalatorTMP.js';
...
function generateWorld() {
    ...
    const balustradeMaterial = new THREE.MeshStandardMaterial({ color: 0x999999, metalness: 0.8, roughness: 0.2 });
 ...
 */
           ...
           /*   if (frontWallRightWidth > 0.01) {
            ...
            const floorGeo = new THREE.PlaneGeometry(SETTINGS.roomSize, totalCorridorLength);
            const floor = new THREE.Mesh(floorGeo, floorMaterial);
            floor.name = `Floor ${i}`;
            floor.rotation.x = -Math.PI / 2;
            floor.position.set(SETTINGS.roomSize / 2, floorY, totalCorridorLength / 2);
            ...
            floorB.position.set(SETTINGS.roomSize / 2, floorY, -16- totalCorridorLength / 2);
            ...
            for (let j = 0; j < SETTINGS.doorsPerSide; j++) {
                ...
                createOuterWallWithWindow(SETTINGS.corridorWidth + SETTINGS.roomSize - wallDepth / 2, floorY + SETTINGS.wallHeight / 2, segmentCenterZ, SETTINGS.corridorSegmentLength, SETTINGS.wallHeight, wallDepth, wallMaterial, opaqueGlassMaterial, glassMaterial, leftRoomId);
                                ...
                                lCeilingB.position.set(roomRXCenter, floorY + SETTINGS.wallHeight + roomCeilingThickness / 2, segmentBCenterZ);
                                lCeilingB.castShadow = true; lCeilingB.receiveShadow = true;
                                lCeilingB.name = `RoomCeiling_B_L_F${i}_D${j}`;
                                ...
                                // Chair for Left Room B
                                //const chairSeatWidth = 0.5, chairSeatDepth = 0.65, chairSeatHeight = 0.5;
                                //const chairBackrestHeight = 0.8, chairBackrestThickness = 0.15;
                                const backWallZ_BL_Chair = segmentBCenterZ + SETTINGS.corridorSegmentLength / 2; // This is the Z of the wall behind the desk
                                const chairZ_BL = 0.1 + (deskBL.position.z + backWallZ_BL_Chair) / 2; // Position chair between desk and back wall
                                const chairX_BL = -(SETTINGS.roomSize/2);
                                const chairY_BL = floorY + chairSeatHeight / 2;
                                const chairBSeat_L = new THREE.Mesh(new THREE.BoxGeometry(chairSeatWidth, chairSeatHeight, chairSeatDepth), deskMaterial);
                                chairBSeat_L.position.set(chairX_BL, chairY_BL, chairZ_BL); // scene.add(chairBSeat_L); worldObjects.push(chairBSeat_L);
                                const backrestB_L = new THREE.Mesh(new THREE.BoxGeometry(chairSeatWidth, chairBackrestHeight, chairBackrestThickness), deskMaterial);
                                backrestB_L.position.set(chairX_BL, chairY_BL + chairBackrestHeight / 2, chairZ_BL + chairSeatDepth / 2 - chairBackrestThickness / 2);
                                // scene.add(backrestB_L); worldObjects.push(backrestB_L);
                                ...
                                leftRoomContents.add(chairBSeat_L); worldObjects.push(chairBSeat_L);
                                leftRoomContents.add(backrestB_L); worldObjects.push(backrestB_L);
                                ...
                                if (isLeftBRoomRedDoor) {
                                    ...
                                    safeBL.position.set(SETTINGS.corridorWidth + SETTINGS.roomSize - safeDepth / 2, floorY + safeHeight / 2, segmentBStartZ + SETTINGS.corridorSegmentLength - safeWidth / 2 - 0.1);
        ...
        // Call the unified escalator creation function from escalatorTMP.js
        createEscalatorsForFloor({
            floorIndex: i,
            scene,
            worldObjects,
            materials: { // Pass all necessary materials
                floorMaterial, // Defined in generateWorld
                escalatorMaterial: window.EscalatorMaterial, // Global
                escalatorEmbarkMaterial: window.EscalatorEmbarkMaterial, // Global
                balustradeMaterial, // Defined in generateWorld
                // Add any other materials createEscalatorsForFloor might need from its original context
            },
            settings: SETTINGS,
            totalCorridorLength,
            lightsRef: lights,
            lightBulbMaterial, // Defined in generateWorld
            escalatorSteps, // Global
            escalatorStarts, // Global
            escalatorEnds, // Global
            createStandardLampFn: createStandardLamp // Pass the function itself
        });
...
function updatePlayer(deltaTime) {
    ...
    const escalatorBoost = calculateEscalatorBoost(cameraObject, playerHeight, escalatorSteps, escalatorStarts, escalatorEnds, SETTINGS, deltaTime);
...
function animate() {
    ...
    if (controls.isLocked) {
        ...
        updateUI();
        // Pass escalatorEnds as the 6th parameter now.
        animateActiveEscalatorSteps(deltaTime, escalatorSteps, escalatorStarts, escalatorEnds, SETTINGS, {
        ...
        const playerStandBox = new THREE.Box3().setFromCenterAndSize(
            new THREE.Vector3(playerPos.x, playerPos.y - playerHeight, playerPos.z), // Centered at player's feet
        ...
        // Call updateEscalatorStepVisuals from escalatorTMP.js
        updateEscalatorStepVisuals(playerPos, playerHeight, playerOnEscalator, escalatorSteps, escalatorStarts, escalatorEnds, SETTINGS, {
            escalatorMaterial: window.EscalatorMaterial,
            escalatorEmbarkMaterial: window.EscalatorEmbarkMaterial
        });
    }
