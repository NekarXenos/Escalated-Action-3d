...
function generateWorld() {
    ...
    // --- Furniture Materials ---
    const deskMaterial = new THREE.MeshStandardMaterial({ color: 0x8B4513 }); // Brown for wood
    const cabinetMaterial = new THREE.MeshStandardMaterial({ color: 0x888888 }); // DarkGray for metal
    const safeMaterial = new THREE.MeshStandardMaterial({ color: 0xCC0000, metalness: 0.8, roughness: 0.3 }); // Red, metallic safe
    const dialMaterial = new THREE.MeshStandardMaterial({ color: 0x333333, metalness: 0.9, roughness: 0.2 }); // Dark metallic dial
    ...
    for (let i = 0; i < SETTINGS.numFloors; i++) {
        ...
        for (let j = 0; j < SETTINGS.doorsPerSide; j++) {
            ...
            const deskWidth = 1.2, deskHeight = 0.75, deskDepth = 0.6; // Desk: Z-width, Y-height, X-depth
            const cabinetWidth = 0.5, cabinetHeight = 1.0, cabinetDepth = 0.6; // Cabinet: Z-width, Y-height, X-depth
            const safeWidth = 0.6, safeHeight = 0.8, safeDepth = 0.5; // Safe: Z-width, Y-height, X-depth
            const dialRadius = 0.08, dialLength = 0.1; // Dial dimensions

            // Safe UserData defaults
            ...
            // Determine if the door for this room segment is the red door
            ...
            // Add Desk to Right Room
            const deskRGeo = new THREE.BoxGeometry(deskDepth, deskHeight, deskWidth);
            const deskR = new THREE.Mesh(deskRGeo, deskMaterial);
            deskR.position.set(
                -SETTINGS.roomSize + deskDepth / 2, // Against outer wall
                floorY + deskHeight / 2,
                segmentCenterZ // Centered in Z
            );
            deskR.castShadow = true; deskR.receiveShadow = true;
            deskR.name = `Desk_R_F${i}_D${j}`;
            scene.add(deskR);
            worldObjects.push(deskR);

            // Add Filing Cabinet to Right Room
            ...
            cabinetR.position.set(
                -SETTINGS.roomSize + cabinetDepth / 2, // Against outer wall
                floorY + cabinetHeight / 2,
                segmentStartZ + cabinetWidth / 2 + 0.1 // In a corner of the segment
            );
            cabinetR.castShadow = true; cabinetR.receiveShadow = true;
            cabinetR.name = `Cabinet_R_F${i}_D${j}`;
            scene.add(cabinetR);
            worldObjects.push(cabinetR);

            // Add Safe to Right Room if it has the red door
            if (isRightRoomRedDoor) {
                ...
                safeR.position.set(
                    -SETTINGS.roomSize + safeDepth / 2, // Against outer wall
                    floorY + safeHeight / 2,
                    segmentStartZ + SETTINGS.corridorSegmentLength - safeWidth / 2 - 0.1 // Opposite corner to cabinet
                );
                safeR.castShadow = true; safeR.receiveShadow = true;
                safeR.name = `Safe_R_F${i}_D${j}`;
                safeR.userData = defaultSafeUserData();
                scene.add(safeR);
                worldObjects.push(safeR);

                // Add Dial to Right Safe
                const dialRGeo = new THREE.ConeGeometry(dialRadius, dialLength, 16);
                const dialR = new THREE.Mesh(dialRGeo, dialMaterial);
                // Position on the front face (+X local direction for right room safe)
                dialR.position.set(safeDepth / 2, 0, 0); // Base of cone on safe surface
                dialR.rotation.z = -Math.PI / 2; // Orient cone to point along +X
                dialR.userData.isSafeDial = true;
                dialR.name = `Dial_Safe_R_F${i}_D${j}`;
                safeR.add(dialR);
                // Note: dialR is implicitly added to worldObjects through safeR for raycasting if recursive is true
            }
            // Create room lamp for every right-side room segment
            ...
            // Left Room Ceiling
            /*if (i === SETTINGS.numFloors-1) {
            const lCeilingGeo = new THREE.PlaneGeometry(SETTINGS.roomSize,  SETTINGS.corridorSegmentLength);
            const lCeiling = new THREE.Mesh(lCeilingGeo, ceilingMaterial);
            lCeiling.rotation.x = -Math.PI / 2; // only when using plane geometry - disable when using box geometry
            lCeiling.position.set(roomLXCenter, floorY + SETTINGS.wallHeight , segmentCenterZ);
            ...
            // Add Desk to Left Room
            const deskLGeo = new THREE.BoxGeometry(deskDepth, deskHeight, deskWidth);
            const deskL = new THREE.Mesh(deskLGeo, deskMaterial);
            deskL.position.set(
                SETTINGS.corridorWidth + SETTINGS.roomSize - deskDepth / 2, // Against outer wall
                floorY + deskHeight / 2,
                segmentCenterZ // Centered in Z
            );
            deskL.castShadow = true; deskL.receiveShadow = true;
            deskL.name = `Desk_L_F${i}_D${j}`;
            scene.add(deskL);
            worldObjects.push(deskL);

            // Add Filing Cabinet to Left Room
            ...
            cabinetL.position.set(
                SETTINGS.corridorWidth + SETTINGS.roomSize - cabinetDepth / 2, // Against outer wall
                floorY + cabinetHeight / 2,
                segmentStartZ + cabinetWidth / 2 + 0.1 // In a corner of the segment
            );
            cabinetL.castShadow = true; cabinetL.receiveShadow = true;
            cabinetL.name = `Cabinet_L_F${i}_D${j}`;
            scene.add(cabinetL);
            worldObjects.push(cabinetL);

            // Add Safe to Left Room if it has the red door
            if (isLeftRoomRedDoor) {
                ...
                safeL.position.set(
                    SETTINGS.corridorWidth + SETTINGS.roomSize - safeDepth / 2, // Against outer wall
                    floorY + safeHeight / 2,
                    segmentStartZ + SETTINGS.corridorSegmentLength - safeWidth / 2 - 0.1 // Opposite corner to cabinet
                );
                safeL.castShadow = true; safeL.receiveShadow = true;
                safeL.name = `Safe_L_F${i}_D${j}`;
                safeL.userData = defaultSafeUserData();
                scene.add(safeL);
                worldObjects.push(safeL);

                // Add Dial to Left Safe
                const dialLGeo = new THREE.ConeGeometry(dialRadius, dialLength, 16);
                const dialL = new THREE.Mesh(dialLGeo, dialMaterial);
                // Position on the front face (-X local direction for left room safe)
                dialL.position.set(-safeDepth / 2, 0, 0); // Base of cone on safe surface
                dialL.rotation.z = Math.PI / 2; // Orient cone to point along -X
                dialL.userData.isSafeDial = true;
                dialL.name = `Dial_Safe_L_F${i}_D${j}`;
                safeL.add(dialL);
                // Note: dialL is implicitly added to worldObjects through safeL for raycasting if recursive is true
            }
            // Create room lamp for every left-side room segment
        ...
        if (i > 0) {
            ...
            const balustradeMaterial = new THREE.MeshStandardMaterial({ color: 0x777777 }); // Gray material
