...
// --- New Enemy Sprite Placeholder Logic ---

// Creates an enemy sprite for the given floor.
// Positions enemy near the center (adjust as needed).
function createEnemyAtFloor(floorIndex, scene) {
    const enemyMaterial = new THREE.SpriteMaterial({ color: 0xff0000 });
    const enemyGeo = new THREE.BoxGeometry(0.5, 1.8, 0.3);
    const enemySprite = new THREE.Mesh(enemyGeo, enemyMaterial);
    //const enemySprite = new THREE.Sprite(enemyMaterial);
    //enemySprite.scale.set(1, 1, 1);
    // Position enemy: use corridor center and offset upward from floor level.
    enemySprite.position.set(SETTINGS.corridorWidth / 2, floorIndex * SETTINGS.floorHeight + 1, rand*totalCorridorLength);
    enemySprite.userData = { floor: floorIndex, lastFireTime: 0, fireInterval: 2000 };
    scene.add(enemySprite);
    enemies.push(enemySprite);
}

// Placeholder function: compute direction from enemy to player and "fire" once in view.
function enemyFire(enemy) {
    const direction = new THREE.Vector3().subVectors(camera.position, enemy.position).normalize();
    console.log(`Enemy on floor ${enemy.userData.floor} fires in direction:`, direction);
    // Optionally, add bullet creation logic here.
}

// Update enemy behavior each frame:
// If enemy is within 20 units of the camera, have it "fire" if enough time has passed.
function updateEnemies(delta) {
    const currentTime = performance.now();
    enemies.forEach(enemy => {
        if (enemy.position.distanceTo(camera.position) < 20) {
            if (currentTime - enemy.userData.lastFireTime > enemy.userData.fireInterval) {
                enemyFire(enemy);
...
function createProjectile(startPosition, direction) {
    ...
    projectile.userData = {
        ...
        spawnTime: clock.getElapsedTime() // clock needs to be initialized before generateWorld
    };
...
// Add Text "A" + floor number to endWallAFar
        ...
        if (i >= 0) { // Office Floors
            ...
            for (let j = 0; j < SETTINGS.doorsPerSide; j++) {
                ...
                // Place one enemy per segment in A-wing corridor
                createEnemy(SETTINGS.corridorWidth / 2, floorY, segmentCenterZ_A, i);
                // Place one enemy per segment in B-wing corridor
                createEnemy(SETTINGS.corridorWidth / 2, floorY, segmentCenterZ_B, i);
            }
        }
...
function resetGame() {
    ...
    // If generateWorld is not called again, enemies need to be re-added if they were cleared.
    // However, if enemies are part of generateWorld, they'd be recreated if generateWorld is called.
    ...
    enemies.forEach(enemy => {
        ...
        if (playerInLOS && distanceToPlayer <= ENEMY_SETTINGS.losMaxDistance) {
            ...
            if (currentTime > enemy.userData.lastShotTime + ENEMY_SETTINGS.fireRate) {
                ...
                firePosition.addScaledVector(forwardVector, ENEMY_SETTINGS.depth / 2 + 0.1);
                // Adjust firePosition.y if gun is not at enemy center height
                // firePosition.y += ENEMY_SETTINGS.height * 0.2; // Example
...
function updateProjectiles(deltaTime) {
    ...
    for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        if (projectile.position.distanceTo(playerBodyCenter) < playerHeight / 2) { // Approx player radius
        ...
        const projectileBox = new THREE.Box3().setFromObject(projectile); // Simpler AABB from object
        for (const wo of worldObjects) {
            if (wo.userData.type === 'enemy' || wo.userData.type === 'projectile' || wo === projectile || !wo.geometry || !wo.geometry.boundingBox) continue;
