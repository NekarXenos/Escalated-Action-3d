...
const buildingWidth = SETTINGS.corridorWidth + (2 * roomSize); // Total width of the building
...
function generateWorld() {
    ...
    const shaftHoleMinX = -roomSize; // X-bounds of the shaft opening in the floor
    const shaftHoleMaxX = SETTINGS.corridorWidth + roomSize; // X-bounds of the shaft opening in the floor
    const shaftHoleMinZ = -SETTINGS.elevatorSize - 4; // New Z for back of shaft hole
    const shaftHoleMaxZ = -4; // New Z for front of shaft hole
    ...
    const buildingMinZ_footprint = -SETTINGS.elevatorSize; // Building front edge. Shaft is now behind this if elevatorSize > 0.
    ...
    RoofWallR.position.set(-roomSize, (SETTINGS.numFloors) * SETTINGS.floorHeight + SETTINGS.wallHeight / 2, -elevatorSize / 2 - 4); // Elevator shaft wall
    ...
    RoofWallL.position.set(SETTINGS.corridorWidth + roomSize, (SETTINGS.numFloors) * SETTINGS.floorHeight + SETTINGS.wallHeight / 2, -elevatorSize / 2 - 4); // Elevator shaft wall
    ...
    const roofEndWallGeo = new THREE.BoxGeometry(buildingWidth, SETTINGS.floorHeight, wallDepth);
    const roofEndWallNear = new THREE.Mesh(roofEndWallGeo, wallMaterial);
    roofEndWallNear.position.set(SETTINGS.corridorWidth / 2, (SETTINGS.numFloors) * SETTINGS.floorHeight + SETTINGS.wallHeight / 2, -SETTINGS.elevatorSize - 4); // Back of shaft
    ...
    //const segmentZ = -SETTINGS.corridorWidth / 2 + doorOffset / 2;
    const topRoofGeo = new THREE.BoxGeometry(buildingWidth, floorDepth, elevatorSize); // Roof dimensions
    //const RoofWallMaterial = new THREE.MeshStandardMaterial({ color: 0xcccccc });
    const topRoof = new THREE.Mesh(topRoofGeo, floorMaterial);
    topRoof.name = `Top Roof over Elevator`; // This is the roof OF the elevator shaft structure
    topRoof.position.set(SETTINGS.corridorWidth/2, (1 + SETTINGS.numFloors) * SETTINGS.floorHeight, -elevatorSize / 2 - 4); // Centered in new shaft Z
    ...
    const roofActualCenterZ = 2 + ((totalCorridorLength + SETTINGS.escalatorLength) / 2);
    ...
    for (let i = -SETTINGS.numBasementFloors; i < SETTINGS.numFloors; i++) {
        ...
        if (i < 0) { // --- Basement Floor Generation ---
           ...
           /*  const connectorBasementFloorGeo = new THREE.BoxGeometry(SETTINGS.corridorWidth, floorDepth, 4);
            const connectorBasementFloor = new THREE.Mesh(connectorBasementFloorGeo, concreteMaterial);
            connectorBasementFloor.position.set(SETTINGS.corridorWidth / 2, floorPanelY, -2); // Centered at Z=-2
 ...
 */
            const connectorBasementCeilingGeo = new THREE.BoxGeometry(SETTINGS.corridorWidth, floorDepth / 2, 4);
            const connectorBasementCeiling = new THREE.Mesh(connectorBasementCeilingGeo, concreteMaterial);
            connectorBasementCeiling.position.set(SETTINGS.corridorWidth / 2, ceilingPanelY, -2); // Centered at Z=-2
           ...
           /*  if (frontWallRightWidth > 0.01) {
            ...
            const connectorFloorGeo = new THREE.BoxGeometry(buildingWidth, floorDepth, 4);
            const connectorFloor = new THREE.Mesh(connectorFloorGeo, floorMaterial);
            connectorFloor.name = `ConnectorFloor_F${i}`;
            connectorFloor.position.set(SETTINGS.corridorWidth / 2, floorY - floorDepth / 2, -2); // Centered at Z=-2
        ...
        const segmentZ = -SETTINGS.corridorWidth / 2 + doorOffset / 2;
        const wallRGeo = new THREE.BoxGeometry(wallDepth, SETTINGS.floorHeight, elevatorSize +8); // Wall depth and height
        //const elevatorWallMaterial = new THREE.MeshStandardMaterial({ color: 0x999999 });
        const wallR = new THREE.Mesh(wallRGeo, wallMaterial);
        wallR.name = `Elevator Right Wall ${i}`;
        wallR.position.set(-roomSize, floorY + SETTINGS.wallHeight / 2, -elevatorSize/2 - 4); // Centered in new shaft Z
        ...
        const LeftWallX = SETTINGS.corridorWidth + roomSize; 

        // Left Wall next to Elevator
        const wallLGeo = new THREE.BoxGeometry(wallDepth, SETTINGS.floorHeight, elevatorSize+8); // Wall depth and height
        const wallL = new THREE.Mesh(wallLGeo, wallMaterial);
        wallL.position.set(LeftWallX, floorY + SETTINGS.wallHeight / 2, -elevatorSize/2 - 4); // Centered in new shaft Z
        ...
        // wall behind elevator
        const endWallGeo = new THREE.BoxGeometry(buildingWidth, SETTINGS.floorHeight, wallDepth);
        const endWallNear = new THREE.Mesh(endWallGeo, wallMaterial);
        endWallNear.position.set(SETTINGS.corridorWidth / 2, floorY + SETTINGS.wallHeight / 2, -SETTINGS.elevatorSize - 4); // New back of shaft
        ...
        capWallNear.position.set(SETTINGS.corridorWidth / 2, floorY + SETTINGS.wallHeight +(SETTINGS.floorHeight - SETTINGS.wallHeight)  / 2, -4); // At new shaft front
    ...
    // Elevator Platform
    const elevatorGeo = new THREE.BoxGeometry(SETTINGS.corridorWidth - 0.2, 0.2, SETTINGS.elevatorSize - 0.2);
    elevator = new THREE.Mesh(elevatorGeo, elevatorMaterial);
    elevator.name = `Elevator Floor`;
    // Position elevator in the new recessed shaft
    elevator.position.set(SETTINGS.corridorWidth / 2, -0.1, -elevatorSize / 2 - 4);
    ...
    const pitGeo = new THREE.BoxGeometry(buildingWidth, pitThickness, SETTINGS.elevatorSize);
    const pitMesh = new THREE.Mesh(pitGeo, concreteMaterial); // Use concrete for the pit
    pitMesh.position.set(SETTINGS.corridorWidth / 2, pitCenterY, -SETTINGS.elevatorSize / 2 - 4); // Centered in new shaft Z
...
function updateElevator(deltaTime) {
    ...
    if (arrived) {
        ...
        if (playerIsOnElevator) {
            playerVelocity.y = 2.0; // Make the player jump slightly
            playerOnGround = false; // Allow the jump to take effect
        }
