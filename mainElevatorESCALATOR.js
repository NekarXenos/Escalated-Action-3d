...
function interact() {
    ...
    if (intersects.length > 0) {
        ...
        if (distance < 3) { // Interaction range
            ...
            // Check if it's still a red door (hasn't been interacted with successfully yet)
            if (doorData.isRed && intersectedDoor.material !== window.blackDoorMaterial) {
                 console.log("SUCCESS! Found the documents!");
                 // Add game winning logic here
                 alert("SUCCESS! You found the documents!");
                 // Change the door to a black door
                 intersectedDoor.material = window.blackDoorMaterial;
                 doorData.isRed = false; // Update its state
            } else {
